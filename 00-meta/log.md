...
- 2026-06-08 — G9 썸네일 자동 이미지 생성 구현(ADR-028 경로 실현). thumbnail_prompt→BizRouter `google/gemini-2.5-flash-image`(나노바나나) 실제 이미지 1장. ① orchestrator.generate_thumbnail(): content 배열서 data:image base64 추출, THUMBNAIL_IMAGE 토글(기본ON)·키없음·실패 모두 None graceful 폴백. ② server.py POST /api/thumbnail: 키 환경변수만(브라우저 노출0, Gamma 프록시 패턴), _run_lock cost_total 레이스 회피, 실패 status:failed 강등. ③ web 비동기 loadThumbnail(): 스켈레톤→<img> 교체, 실패 시 프롬프트 텍스트 폴백, _thumbSeq 레이스가드, 샘플 폴백 미호출. 화면 표시만(State/파일 저장 안 함). 검증: railway run 단위(2.9MB b64)·/api/thumbnail 엔드포인트(로컬+prod)·프론트 시각(1024x1024 img 렌더+스켈레톤, 로컬+prod e2e)·회귀 89 PASS. push+railway up 재배포(자동배포 미트리거 회피). YAGNI 제외(메이커-체커·의도기획기·Pydantic).

- 2026-06-08 — /propose-research(Gemini 이미지 생성 파이프라인 설계 가이드) → 신규 0건(ADR-028). 사용자 "gemini-2.5-flash-image 썸네일 생성" 요청 동반. 가이드=Google GenAI SDK·Vertex·Pydantic·async 전제라 BizRouter 경유 불가·의존성0 위반(ADR-007/013/014 영구거부 100% 정합). ADR-014/015/016/019/023 동형(도구/방법론 가이드=신규0). Phase B(haiku) ACCEPT(missed none, self_rationalization LOW). G9(이미지 생성 자동드롭) 결손은 실재하나 딥리서치 아닌 코드작업—thumbnail_prompt는 생성되나 실제 이미지생성 코드 0건. 사용자 요청=G9 신호. 향후 구현경로: thumbnail_prompt→BizRouter 이미지모델 urllib POST→이미지 URL 표시(Gamma ADR-020 패턴), 가이드 프롬프트템플릿만 참고. 거부: 메이커-체커루프·의도기획기·Pydantic=엔터프라이즈 과설계 YAGNI.
- 2026-06-08 — 웹 데모 스크립트 가독성 렌더링: 결과 화면 스크립트가 한 덩어리 평문(textContent)이라 '벽'으로 읽기 힘들던 것 수정. renderScript() 신설 — ① esc 먼저(XSS 차단) ② 문장 단위 줄바꿈(종결부호 뒤, 소수점 보존) ③ 결정론 하이라이트(수치 <b>·CTA <mark> 정규식). index.html 379/478줄 textContent→innerHTML(renderScript) 교체 + CSS(line-height 1.9·mark 칩). 프론트만(LLM·백엔드 무변경). Playwright 검증 PASS: 줄바꿈·수치/CTA 하이라이트 시각확인 + XSS 안전(<script>·<img onerror> 주입 미실행, &lt;escape&gt; 확인) + JS에러 0. 회귀 79 PASS(프론트 비의존).
- 2026-06-08 — /squeeze-report 두 트렌드 생태계 가이드(한국판+글로벌판) 본문화(ADR-027). 분석/판정 haiku 분리: 후보 5건 → ACCEPT 2/DEFER 1/REJECT 2 + 거부확정 4. ✅C1(코드): _trend_scan 글로벌 힌트를 2갈래로—기술 1차담론(HN·Reddit·Karpathy)+비개발자 성과서사 진원지(#BuildInPublic·Indie Hackers·Starter Story)·48시간 큐레이션(Ben's Bites·The Rundown). ADR-024 후속 완성. ✅C2(노트): 시장구조 노트 서구권 대조 한 줄→글로벌 소스 4계층 표+진원지 경로+서구 B2B AIAA vs 한국 1인SaaS 구조+한국 독자 특이점(정직통계·기수제). 구독자수·매출 0. ⏸️C3 DEFER(Kotlin DSL 아키텍처=비개발자 쇼츠엔 과한 시니어기술 YAGNI). ❌C4/C5 REJECT(전문가토론 출처=범위밖). ❌R1~4 거부확정(자기신고MRR·구독자·ARR 추정 ADR-008/011). 판정 haiku가 C3를 자기합리화 차단(분석은 끌렸으나). 회귀 79 PASS.
- 2026-06-08 — /propose-research(글로벌 비개발자 AI 코딩 트렌드 정보 생태계 가이드) → 신규 0건·정정 0건·본문화 squeeze 위임(ADR-026). ★주의: 직전 ADR-025의 한국판과 다른 글로벌판 파일. 한국판=PROMPT_트렌드소스지형 답(answered), 이 글로벌판=그 답 아니라 ADR-024(글로벌 트렌드 확장) 후속 결손. 결손 실재: 시장구조 노트 서구권 대조 'Liam Ottley B2B' 한 줄뿐(글로벌 소스 4계층·디렉토리·진원지 0건), _trend_scan 글로벌 힌트가 HN·Reddit·Karpathy만(비개발자 특화 글로벌 소스 미명시). Phase B(haiku) ACCEPT(missed none, self_rationalization LOW). 본문화 위임: 가이드 검증신뢰도 표 자기신고·단일출처·과거수치 4건(Lovable ARR2억불·Bolt ARR4천만불·Hassam$25K MRR·지피터스33만명2023)은 ADR-008/011로 거르고 검증된 사실(소스계층·채널포지션·진원지·서구B2B vs 한국1인SaaS)만. ADR-024 연결: 글로벌 소스 계층이 _trend_scan 힌트 정밀화 1차자료(squeeze 후).
- 2026-06-08 — /propose-research(비개발자 AI 코딩 트렌드 정보 생태계 분석 가이드) → 신규 0건 + PROMPT 정정 1건(ADR-025). 가이드가 직전 작성한 PROMPT_한국 바이브코딩 트렌드 소스 지형의 딥리서치 답변임을 확인(요구 6항목 §1~§7 100% 충족). Phase B(haiku) 전부 ACCEPT(missed none, self_rationalization LOW). 정정: 그 PROMPT status draft→answered + answered_by. 결손(시장구조 노트에 소스계층·디스콰이엇·지피터스 0건)은 실재하나 본문화는 propose-research 아닌 /squeeze-report 위임(가이드 §7이 자기신고·교차검증실패 3건 정직 분류—조코딩48만·14만3천원·10개월손익분기, ADR-008/011로 거를 대상). 검증된 사실(소스계층·채널포지션·포맷·훅·진원지·서구권대비)만 채택. ADR-024 연결: §1 소스계층(디스콰이엇·지피터스 2~5일 최속)이 _trend_scan 힌트 정밀화 1차자료.
- 2026-06-08 — 트렌드 감지 글로벌 확장(ADR-024): "왜 한국에만 편향? 글로벌이 낫지 않나" 지적 반영. _trend_scan/_trend_deepen이 "한국의 비개발자·바이브코딩"으로 좁혀 글로벌 1차 트렌드를 놓치고 '한 박자 늦은' 것만 잡던 결함 수정. 핵심: 수집=글로벌(넓게 감지, HN·Reddit·Karpathy 1차 담론), 산출=한국어 쇼츠(고정). 접근법 A(프롬프트 텍스트만): scan은 글로벌 진원지 우선 감지+한국 소스 해석 참고(힌트)+키워드는 한국 비개발자가 쓸 주제로 번역(시니어 전용 기술→비개발자 업무 치환), deepen은 글로벌 진원지 명시+한국 업무·수익화 관점 detail. 2계층 힌트. Sonar search_domain_filter 불가(ADR-013)라 프롬프트 텍스트로만 힌트(자유탐색 유지). 비용·구조 무변경(콜 수 동일), 회귀 orc67+srv12=79 PASS(프롬프트 비의존). 실 동작 프로덕션 e2e 검증.
- 2026-06-08 — /propose-research(프레젠테이션 기획서 작성 가이드 116줄) → 신규 딥리서치 0건 + agent_gamma_prompt 프롬프트 보강(ADR-023). Phase A "0건" → Phase B(haiku) CONFIRM(self_rationalization LOW, missed none). 가이드 두 부류: ①PPT 소프트웨어 조작(A4 29.7cm·디자인탭·그림서식 필터)=Gamma API 일임이라 우리 무관, ②슬라이드 구성 원리(두괄식·메시지형 헤드라인·1슬라이드1메시지·역순압축·패널)=출처13개 갖춘 검증 방법론이라 추가 딥리서치 불요(ADR-015/016 동형). ★코드 개선: ②원리가 agent_gamma_prompt 시스템프롬프트에 부재 확인→가이드 검증답을 외부조사 없이 직접 주입(두괄식·메시지형 헤드라인·1슬라이드1메시지·역순·패널 강조). 딥리서치 산출물 아닌 코드작업으로 분리. 회귀 orc67+srv12=79 PASS(프롬프트 문자열은 테스트 비의존). 실 LLM 동작은 프로덕션 /pd e2e 검증.
