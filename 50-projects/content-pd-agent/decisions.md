...
## ADR-024 — 트렌드 감지를 글로벌로 확장 (글로벌 진원지 감지 → 한국 비개발자 번역)
"왜 한국에만 편향? 글로벌로 하면 더 좋지 않나" 사용자 지적. `_trend_scan`/`_trend_deepen` 프롬프트가 "한국의 비개발자·바이브코딩"으로 좁혀져 **글로벌 1차 트렌드를 놓치고 "한 박자 늦은(이미 한국까지 흘러온)" 트렌드만 잡던** 결함을 수정. 트렌드는 글로벌(HN·Reddit·Karpathy)에서 먼저 터져 한국(긱뉴스·요즘IT)으로 시차를 두고 흐른다 — 글로벌 1차 감지가 콘텐츠 선점 우위.
- **핵심 구분:** 수집 언어 ≠ 산출 언어. **수집=글로벌(넓게 감지), 산출=한국어 쇼츠(고정).** 한국 편향은 산출물 타깃 정렬이라 옳지만, 그게 수집까지 좁힌 게 결함.
- **접근법 A(프롬프트 텍스트만 수정):** `_trend_scan` — 감지 범위를 "글로벌 1차 담론 우선 + 한국 소스 해석 참고(강제 아닌 힌트)"로, 키워드는 "한국 비개발자·1인 창업가가 쓸 수 있는 주제로 번역"(시니어 전용 기술→비개발자 업무 치환). `_trend_deepen` — "글로벌 진원지 명시 + 한국 비개발자 업무·수익화 관점 번역" detail. **거부:** 소스 상수 분리(YAGNI — 소스 5~6개로 적음, 딥리서치로 소스 지도 확정 시점에) / `TREND_SCOPE` env 토글(과설계 — 산출 한국어 고정이라 "글로벌 감지+한국 변환"이 항상 정답).
- **결정(사용자 확정):** 글로벌 감지 + 한국 타깃 변환 / 2계층 힌트(글로벌 진원지 + 한국 해석 참고).
- **Sonar 제약 정합(ADR-013):** `search_domain_filter`는 BizRouter 경유 불가 → 소스 힌트는 **프롬프트 텍스트로만**(강제 아닌 우선 참고). 자유 탐색 여지 유지(힌트 밖 새 소스 발굴 가능).
- **비용·구조 무변경:** 콜 수 동일(scan 1 + deepen 1), 함수 시그니처·반환·캐시·테스트 전부 무변경. 회귀 orc 67+srv 12=79 PASS(프롬프트 문자열 비의존). 실 동작은 프로덕션 /api/suggest e2e로 검증(글로벌 진원지 키워드·출처 반영 확인).
- **연결:** PROMPT_한국 바이브코딩 트렌드 소스 지형 딥리서치(ADR 기록 없음, research/) 결과가 오면 소스 힌트를 그 검증 목록으로 정밀화 가능(현재는 핵심 소스 직접 명시).

## ADR-025 — `비개발자 AI 코딩 트렌드 정보 생태계 분석`(가이드) = PROMPT 딥리서치 답변 (정정 + 본문화 위임)
`/propose-research` 인자 가이드가 **직전(ADR 기록 없음) 작성한 PROMPT_한국 바이브코딩 트렌드 소스 지형의 딥리서치 답변**임을 확인. 그 PROMPT가 요구한 6개 항목(① 소스 계층 분류 ② 한국 채널 표 ③ 신속도 평가 ④ 진원지 추적 ⑤ 서구권 대비 ⑥ 검증 신뢰도 표기)을 가이드 §1~§7이 100% 충족. **Phase A "신규 0건 + 정정 1건 + 본문화는 squeeze 위임" → Phase B(독립 haiku) 전부 ACCEPT, missed_gaps none, self_rationalization LOW.**
- **정정(C1):** `PROMPT_한국 바이브코딩 트렌드 소스 지형.md` `status: draft → answered` + `answered_by: "비개발자 AI 코딩 트렌드 정보 생태계 분석.md"`. 다른 answered PROMPT(쇼츠 정량·채널 성장)와 동일 패턴.
- **신규 딥리서치 0건:** 가이드가 이미 그 PROMPT의 최종 답. 미답 영역 0건.
- **결손 실재:** 시장구조 노트(58줄)에 가이드의 핵심(소스 계층 4단계·디스콰이엇·지피터스·신속도 맵·진원지·서구권 대비) 0건 → 본문화 가치 있음. 단 **본문화는 propose-research가 아니라 `/squeeze-report` 영역**(사실성 분리)이라 위임.
- **본문화 시 거를 대상(ADR-008/011 정합):** 가이드 §7 검증신뢰도 표가 이미 자기신고·교차검증실패 3건(조코딩 48만 구독·Hitpaw 14만3천원 매출·디스콰이엇 10개월 손익분기점)을 정직 분류. squeeze-report에서 이 수치는 거르고 **검증된 사실(소스 계층·채널 포지션·포맷·훅 패턴·진원지·서구권 대비)만 채택**.
- **ADR-024 연결:** 가이드 §1 소스 계층("디스콰이엇·지피터스가 가장 빠른 진원지, 2~5일")은 ADR-024에서 프롬프트에 박은 소스 힌트를 정밀화할 1차 자료. squeeze-report 본문화 시 `_trend_scan` 힌트 갱신 후보로 연결 가능.
- **결론:** propose-research 산출물 = 정정 1건. 신규 페어 0건. 가이드 본문화는 `/squeeze-report`로 후속. 재호출 시 이 ADR로 중복 처리 차단(answered 상태가 1차 가드).

## ADR-026 — `글로벌 비개발자 AI 코딩 트렌드 정보 생태계`(가이드) = ADR-024 후속 결손 (신규 0건, 본문화 위임)
`/propose-research` 인자 가이드. **직전 ADR-025의 한국판(`비개발자 AI 코딩...`)과 다른 파일**임에 주의 — 이건 **글로벌판**(`글로벌 비개발자 AI 코딩...`)이다. 한국판은 `PROMPT_한국 바이브코딩 트렌드 소스 지형`의 답(한국 소스 우선)이라 ADR-025로 answered 처리됨. **이 글로벌판은 그 PROMPT의 답이 아니라**(그 프롬프트 검증기준 §5가 "한국어 소스 우선, 글로벌은 출발점만"으로 한정), **ADR-024(트렌드 감지 글로벌 확장)가 만든 새 결손 — "글로벌 비개발자 소스 지형"을 채우는 별도 자료**다. **Phase A "신규 0건 + 본문화 squeeze 위임 + 정정 0건" → Phase B(독립 haiku) ACCEPT(missed none, self_rationalization LOW).**
- **신규 딥리서치 0건:** 가이드가 이미 글로벌 소스 지형의 충실한 답(출처 47개, §검증 신뢰도 표로 자체 검증). 추가 조사 불요.
- **결손 실재(ADR-024 후속):** ① 시장구조 노트의 "서구권 대조"는 "Liam Ottley B2B" **한 줄**뿐 — 글로벌 소스 4계층(1차 담론→큐레이션 미디어→비주얼 튜토리얼→수익화 니치)·구체 디렉토리(Ben's Bites·Starter Story·WeAreNoCode·No Code MBA·The Rundown·Moritz Kremb·Sabrina.dev)·성과 서사 진원지(BuildInPublic→Indie Hackers/Starter Story→유튜버) 0건. ② `_trend_scan` 글로벌 힌트는 현재 "Hacker News·Reddit·Karpathy"만 — **비개발자 특화 글로벌 소스 미명시.** 이 가이드가 그 정밀화의 1차 자료.
- **본문화 /squeeze-report 위임:** 가이드 §검증 신뢰도 표가 자기신고·단일출처·과거수치 **4건**(Lovable ARR 2억불·Bolt ARR 4천만불 단일출처, Hassam $25K MRR 자기신고, 지피터스 33만명 2023 과거수치)을 정직 분류. squeeze에서 ADR-008/011로 거르고 **검증된 사실**(소스 4계층·채널 포지션·포맷·훅·진원지 경로·서구 B2B AIAA vs 한국 1인 SaaS 대조)만 채택. Liam 80.1만 구독은 교차검증됐으나 ADR-008 구독자수 거부 정합상 포지션만.
- **ADR-024 연결(핵심 가치):** 이 가이드의 글로벌 소스 계층(BuildInPublic·Indie Hackers·Starter Story가 성과 서사 진원지, Ben's Bites·Rundown이 48시간 내 큐레이션)이 squeeze 본문화 시 `_trend_scan` 글로벌 힌트를 "HN·Reddit·Karpathy"에서 "비개발자 특화 글로벌 소스"로 정밀화하는 직접 근거. 단 search_domain_filter 불가(ADR-013)라 프롬프트 텍스트 힌트로만.
- **결론:** 신규 페어 0건·정정 0건. 본문화는 `/squeeze-report` 후속(시장구조 노트 서구권 절 확장 + `_trend_scan` 힌트 정밀화). 재호출 시 이 ADR로 재추출 차단.

## ADR-027 — 두 트렌드 생태계 가이드 squeeze 본문화 (글로벌 소스 지형 + _trend_scan 정밀화)
ADR-025/026이 위임한 본문화를 `/squeeze-report`(분석/판정 haiku 분리)로 실행. 두 보고서(한국판·글로벌판 생태계 분석)에서 후보 5건 추출 → 판정 **ACCEPT 2 / DEFER 1 / REJECT 2 + 거부확정 4**.
- **✅ C1 채택(코드, _trend_scan 정밀화):** 글로벌 힌트를 2갈래로 — ① 기술 1차 담론(HN·Reddit·Karpathy 유지) ② **비개발자 성과 서사 진원지(#BuildInPublic·Indie Hackers·Starter Story)·48시간 큐레이션(Ben's Bites·The Rundown)** 추가. ADR-024 후속 완성. 검증된 소스명만(자기신고 수치 0). 회귀 orc 67+srv 12=79 PASS(프롬프트 비의존).
- **✅ C2 채택(지식노트):** [[한국 AI 교육 유튜브 시장 구조]]의 "서구권 대조" 한 줄을 **글로벌 소스 4계층 표 + 성과 서사 진원지 경로 + 서구 B2B AIAA(서비스→소프트웨어 사다리) vs 한국 1인 SaaS·무인 부업 구조 + 한국 독자 특이점(정직한 통계 선호·기수제 챌린지)**로 확장. 구독자수·매출 0건(ADR-008/011 정합).
- **⏸️ C3 DEFER(아키텍처 노트):** 한국 지피터스의 Obsidian·Excalidraw·Kotlin DSL 멱등성 아키텍처. 기술은 검증됐으나 **비개발자 쇼츠 콘텐츠 PD엔 과한 시니어 기술(YAGNI)** — 판정 haiku가 "콘텐츠 적합성 미확정"으로 보류. 사용자 신호 시.
- **❌ C4·C5 REJECT(범위 밖):** 출처가 전문가토론 보고서라 이번 squeeze(두 생태계 분석) 대상 아님. 별도 세션.
- **❌ R1~R4 거부 확정(ADR-008/011):** Hassam $25K MRR(자기신고)·Liam 80.1만 구독·지피터스 33만(과거수치)·Lovable 2억불/Bolt 4천만불 ARR(단일출처 추정). 전부 영구거부 정합 — 포지션·포맷·진원지 경로만 채택, 수치 0.
- **분석/판정 분리 효과:** 분석 haiku가 C3(DSL 아키텍처)를 "검증된 구조"라며 끌렸으나, 판정 haiku가 "비개발자 쇼츠에 시니어 아키텍처는 과설계"로 DEFER 차단 — 자기 합리화 방지 작동.
- **검증:** 회귀 79 PASS. 실 동작은 프로덕션 /api/suggest e2e(진원지 힌트 반영). C2는 지식노트(커밋 대상, Obsidian Sync 아닌 git 추적).

## ADR-028 — `Gemini 이미지 생성 파이프라인 설계`(가이드) — propose-research 0건, G9는 BizRouter 경유 코드작업
사용자가 "gemini-2.5-flash-image로 썸네일 이미지 생성" 요청 + 인자 가이드(Gemini 2.5 Flash Image 파이프라인·메이커-체커 오케스트레이션 설계). propose-research로 점검. **Phase A "신규 딥리서치 0건 + G9는 코드작업" → Phase B(독립 haiku) ACCEPT(missed none, self_rationalization LOW).** ADR-014/015/016/019/023(도구/방법론 가이드 = 신규 0건) 동형.
- **딥리서치 0건:** 가이드는 **Google GenAI SDK(`google-genai`)·Vertex AI(enterprise)·Pydantic `response_schema`·`types.GenerateContentConfig`·async `aio.models`** 전제 — **BizRouter 경유 불가·의존성 0 위반**. ADR-007/013/014 "Google SDK 전용 영구거부"와 100% 정합. 모델 제원(1장 $0.039·1290토큰·해상도)·프롬프트 템플릿은 검증된 사실이라 추가 조사 불요.
- **G9 결손 실재(코드, 딥리서치 아님):** domain-priorities G9(이미지 생성 자동드롭) = "🔵 사용자 신호, BizRouter 이미지 모델 가용, 유료 키 결단만". `thumbnail_prompt`는 Creator가 생성하나(payload 필드) **실제 이미지 생성 코드 0건**. 사용자 요청 = G9 신호 도착. 단 이건 propose-research 산출물(딥리서치)이 아니라 **기능 구현**.
- **G9 구현 경로(향후, BizRouter 경유·의존성 0):** ① `thumbnail_prompt`를 BizRouter 이미지 모델(`google/gemini-2.5-flash-image` 등 가용 확인)에 urllib로 POST → 이미지 URL/base64 수신 → 프론트 표시. ② 가이드의 **프롬프트 템플릿(피사체·조명·구도·화풍)만 참고**해 thumbnail_prompt 품질 향상. **거부:** 메이커-체커 루프(생성→비평→재생성)·의도기획기·Pydantic 구조화출력 = 엔터프라이즈 과설계(YAGNI, 데모는 1콜 생성 충분). Google SDK 전부(임포트 불가).
- **결론:** 신규 페어 0건·정정 0건. G9 이미지 생성은 별도 기능 작업(Gamma 통합 ADR-020 패턴: server.py 프록시·키 환경변수·graceful). 재호출 시 이 ADR로 재추출 차단.
